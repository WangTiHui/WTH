                ...
                <ul class="ul">
                    ...
                    <li v-for="item in fileImgList" :key="item">
                        <div style="width:150px;height:150px;">
                            ...
                            <label class="label"><i class="el-icon-upload-success el-icon-check"></i></label>
                        </div>
                    </li>
                    </template>
...
import { type } from 'os';
	export default {
	    ...
	    props:{
	    	...
	    	uploadUrl:{
	    		type: String,
	    		default:'/api/common/upload-image'
            },
            // 图片的类型 png,jpg
	    	typeImg:{
	    		type: String,
                ...
                setTimeout(() => {
                    this.$nextTick(()=>{
                        if(productImgs.length>0){
                            ...
                            this.fileImgList = Array.from(new Set(this.fileImgList,productImgs))
                        }
                        this.showThis = true
                        this.isLimitLength()
                    })
        ...
        //   图片类型限制
            ...
            }else if(this.typeImg=='image/jpg'||this.typeImg == 'image/png'||this.typeImg=='image/jpeg'||this.typeImg=='image/gif'){
	        ...
	        if (!isLt2M) {
                if(this.typeSize<1){
                    this.$message.error('上传图片大小不能超过 '+this.typeSize*1024+'K!');
                    return false;
                }else{
            ...
            return new Promise(function(resolve, reject) {
                ...
                reader.onload = function(event) {
                    ...
                    image.onload = function () {
                        ...
                        if(setWidthAndHeight == "" ||setWidthAndHeight=='undefined'||setWidthAndHeight==undefined){
                        ...
                        if(heightBasic==0||widthBasic==0||heightBasic==-1||widthBasic==-1){
          ...
          handleAvatarSuccess(res, file) {//图片上传成功
            if(res.code==200){
                let urlListCopy = []
                urlListCopy.push(res.data.upload_url||res.data.basics_url||res.data.backend_url||res.data.tutorial_url||res.data)
                urlListCopy.forEach(e=>{
                    this.fileImgList.push(e)
                    if (!this.showImgs) { // 返回字符
                        this.$emit('urlList', e);
                    }
                })
